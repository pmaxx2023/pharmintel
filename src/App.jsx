...
const TOPICS = [
  { label: "Cardinal Health", q: "Cardinal Health" },
  { label: "Cencora", q: "Cencora" },
  { label: "Drug Shortages", q: "drug shortage pharmacy" },
  { label: "DSCSA", q: "DSCSA compliance" },
  { label: "Generic Drug Market", q: "generic drug pricing" },
  { label: "Independent Pharmacy", q: "independent pharmacy" },
  { label: "McKesson Connect", q: "McKesson pharmacy" },
  { label: "Pharma Ad Regulation", q: "pharmaceutical advertising regulation" },
  { label: "Pharmacy Technology", q: "pharmacy technology automation" },
  { label: "Retail Media", q: "retail media pharmacy" },
  { label: "Specialty Distribution", q: "specialty pharmacy distribution" },
  { label: "Strategic Accounts", q: "pharmacy wholesale accounts" },
];
const WORKFLOWS = [
  { label: "Ordering", q: "pharmacy ordering wholesale", desc: "How pharmacies place & receive orders" },
  { label: "Returns", q: "pharmacy drug returns", desc: "Expired, damaged, recalled product" },
  { label: "Chargebacks", q: "pharmaceutical chargeback", desc: "Contract price disputes with mfrs" },
  { label: "Inventory Mgmt", q: "pharmacy inventory management", desc: "Par levels, cycle counts, shrinkage" },
  { label: "Contract Pricing", q: "pharmacy GPO contract pricing", desc: "GPO contracts, WAC, cost-plus" },
  { label: "Invoice & AP", q: "pharmacy wholesale invoice payment", desc: "Payment terms, credits, reconciliation" },
  { label: "Backorders", q: "drug backorder shortage", desc: "Allocation, substitution, comms" },
  { label: "Controlled Substances", q: "controlled substance pharmacy DEA", desc: "CSOS, DEA reporting, thresholds" },
  { label: "Drop Ship", q: "pharmacy drop ship manufacturer", desc: "Direct-from-mfr fulfillment" },
  { label: "Formulary Mgmt", q: "pharmacy formulary management", desc: "Preferred products, therapeutic subs" },
  { label: "Rebates", q: "pharmaceutical rebate", desc: "Mfr rebates, compliance, tracking" },
  { label: "EDI Integration", q: "pharmacy EDI electronic ordering", desc: "850/855/856/810, AS2 connections" },
];
const SEGMENTS = [
  { label: "Strategic Accounts", q: "pharmacy chain wholesale", desc: "Large chain & regional", accounts: [
    { name: "Wegmans", q: "Wegmans pharmacy" },
    { name: "H-E-B", q: "HEB pharmacy" },
    { name: "Albertsons", q: "Albertsons pharmacy" },
    { name: "CVS", q: "CVS pharmacy" },
    { name: "Costco", q: "Costco pharmacy" },
    { name: "Kinney Drugs", q: "Kinney Drugs" },
    { name: "Optum", q: "Optum pharmacy" },
    { name: "ProAct", q: "ProAct pharmacy" },
  ]},
  { label: "Independent Pharmacy", q: "independent pharmacy", desc: "Single-location & small chain" },
  { label: "Health Mart", q: "Health Mart McKesson", desc: "McKesson franchise network" },
  { label: "Hospitals", q: "hospital pharmacy supply chain", desc: "Health system & acute care" },
  { label: "Government / VA", q: "VA pharmacy 340B", desc: "Federal, VA, 340B programs" },
  { label: "Specialty Pharmacy", q: "specialty pharmacy", desc: "Biologics, oncology, limited dist." },
];
const VOICES_LIST = [
...
export default function PharmIntel() {
  ...
  const fetchNews = useCallback(async (currentLens, limit) => {
    ...
    const p = pj(t);
    return p.length ? { topline: p[0]?.title || "", items: p } : null;
  }, []);
  ...
  // ── Fetch Voices (AI, sequential to respect rate limits) ──
  const runVoices = useCallback(async (currentLens) => {
    ...
    // 2. Fetch — one at a time with delays
    const iv = sL("voices");
    const topic = currentLens?.label || "";
    const ab = topic ? ` about "${topic}"` : "";
    const sa = topic || "pharmaceutical distribution";
    const collected = [];
    try {
      for (let i = 0; i < Math.min(5, VOICES_LIST.length); i++) {
        if (i > 0) await wait(12000);
        const v = VOICES_LIST[i];
        try {
          const t = await api(SYS + `\nReturn a JSON array of 1-2 recent public statements by ${v.name} (${v.title})${ab}. Each object: "quote"(the actual statement, 1-2 sentences),"context"(where and when they said it),"url"(article URL),"date"(YYYY-MM-DD). If no statements found, return []. Only valid JSON.`,
            `Find recent public statements, quotes, or comments by ${v.name} about ${sa}. Look for earnings calls, interviews, conference speeches, press releases. Last 90 days.`);
          const stmts = pj(t);
          if (stmts.length) {
            const entry = { ...v, stmts };
  ...
  const runFree = useCallback(async (query) => {
    ...
    const iv = sL("news");
    try {
      const t = await api(SYS + `\nReturn JSON: {"topline":"1 factual sentence","items":[4 items, each with: ${SCH}]}. Each summary MUST be 3-4 sentences. Only valid JSON.`,
        `Search for recent developments about ${query} in pharmaceutical distribution. For each result, write a detailed summary with specific facts. Last 90 days. Include URLs.`);
      try {
        const obj = JSON.parse(t.replace(/```json|```/g, "").trim().match(/\{[\s\S]*\}/)?.[0] || "{}");
